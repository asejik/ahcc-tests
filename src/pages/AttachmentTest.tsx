...
import { ATTACHMENT_QUESTIONS, ATTACHMENT_DESCRIPTIONS, type AttachmentStyle } from '../lib/attachmentData';
import { calculateAttachmentScores, generateAttachmentPrompt } from '../lib/attachmentLogic';
import { getGenericAnalysis } from '../lib/gemini';
import { saveAssessment } from '../lib/services';
import { useAssessmentStore } from '../store/assessmentStore';
import { Button } from '../components/ui/Button';
import { ArrowRight, ArrowLeft, Download, RefreshCcw, HeartHandshake } from 'lucide-react';
...
export const AttachmentTest = () => {
  ...
  const currentQ = ATTACHMENT_QUESTIONS[currentIdx];
  const totalQ = ATTACHMENT_QUESTIONS.length;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const calculated = calculateAttachmentScores(answers);
      const aiPrompt = generateAttachmentPrompt(userInfo.name, calculated);
      const aiAnalysis = await getGenericAnalysis(aiPrompt);

      const fullResult = {
        primary: calculated.primary,
        secondary: calculated.secondary || "N/A",
        scores: calculated.scores,
        isBlend: calculated.isMixed,
        analysis: aiAnalysis,
        type: "Attachment"
      };
    ...
    return (
      <div className="min-h-screen pt-20 pb-20 px-4 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <p className="text-rose-400 font-medium mb-2 uppercase tracking-widest text-sm">Assessment Complete</p>
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-white mb-6">Your Attachment Style</h1>
          ...
          <div className="inline-block bg-white/5 border border-white/10 rounded-2xl p-8 mb-8">
            ...
            {resultData.isBlend && (
              <p className="text-rose-300 font-medium">Mixed with {resultData.secondary}</p>
            )}
            <p className="text-slate-400 mt-4 max-w-xl mx-auto leading-relaxed">
              {ATTACHMENT_DESCRIPTIONS[resultData.primary as AttachmentStyle]}
            </p>
          </div>

          <div className="max-w-3xl mx-auto bg-rose-500/10 border border-rose-500/20 rounded-xl p-6 text-left relative">
            <HeartHandshake className="absolute top-4 right-4 text-rose-500/20 w-12 h-12" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 max-w-3xl mx-auto">
          {Object.entries(resultData.scores).map(([style, score]) => (
            <div key={style} className={`glass-panel p-5 rounded-xl border-l-4 ${style === resultData.primary ? 'border-rose-500 bg-rose-500/5' : 'border-slate-600'}`}>
              <div className="flex justify-between items-center mb-2">
                <span className={`font-bold ${style === resultData.primary ? 'text-white' : 'text-slate-400'}`}>{style}</span>
                <span className="text-xs text-slate-500">{score as number} / 30</span>
              </div>
              <div className="w-full h-2 bg-slate-800 rounded-full">
                <div
                  className={`h-full rounded-full ${style === resultData.primary ? 'bg-rose-500' : 'bg-slate-600'}`}
                ...
                <button
                  ...
                  className={`
                    ...
                    ${answers[currentQ.id] === val
                      ? 'bg-rose-600 border-rose-400 text-white scale-105 shadow-xl shadow-rose-500/20 z-10'
        ...
        <div className="flex justify-between mt-12 pt-8 border-t border-white/5 items-center">
          ...
          {currentIdx === totalQ - 1 ? (
            <Button
              ...
              className="bg-rose-600 hover:bg-rose-500 text-white px-8"
            >
