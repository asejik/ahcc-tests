...
import { LOVE_LANGUAGE_QUESTIONS, LOVE_LANGUAGE_DESCRIPTIONS, type LoveLanguageType } from '../lib/loveLanguageData';
import { calculateLoveLanguageScores, generateLoveLanguagePrompt } from '../lib/loveLanguageLogic';
import { getGenericAnalysis } from '../lib/gemini';
import { saveAssessment } from '../lib/services';
import { useAssessmentStore } from '../store/assessmentStore';
import { Button } from '../components/ui/Button';
import { ArrowRight, ArrowLeft, Download, RefreshCcw, Heart } from 'lucide-react';
...
export const LoveLanguageTest = () => {
  ...
  const currentQ = LOVE_LANGUAGE_QUESTIONS[currentIdx];
  const totalQ = LOVE_LANGUAGE_QUESTIONS.length;
  const progress = ((currentIdx + 1) / totalQ) * 100;

  const handleSelect = (value: LoveLanguageType) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const calculated = calculateLoveLanguageScores(answers);
      const aiPrompt = generateLoveLanguagePrompt(userInfo.name, calculated);
    ...
    return (
      <div className="min-h-screen pt-20 pb-20 px-4 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <p className="text-pink-400 font-medium mb-2 uppercase tracking-widest text-sm">Assessment Complete</p>
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-white mb-6">Your Love Language</h1>
          ...
          <div className="inline-block bg-white/5 border border-white/10 rounded-2xl p-8 mb-8">
            ...
            <p className="text-pink-300 font-medium">Secondary: {resultData.secondary}</p>
            <p className="text-slate-400 mt-4 max-w-xl mx-auto leading-relaxed">
              {LOVE_LANGUAGE_DESCRIPTIONS[resultData.primary as LoveLanguageType]}
            </p>
          </div>

          {/* AI Analysis */}
          <div className="max-w-3xl mx-auto bg-pink-500/10 border border-pink-500/20 rounded-xl p-6 text-left relative">
            <Heart className="absolute top-4 right-4 text-pink-500/20 w-12 h-12" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 max-w-3xl mx-auto">
          {Object.entries(resultData.scores).map(([lang, score]) => (
            <div key={lang} className={`glass-panel p-5 rounded-xl border-l-4 ${lang === resultData.primary ? 'border-pink-500 bg-pink-500/5' : 'border-slate-600'}`}>
              <div className="flex justify-between items-center mb-2">
                <span className={`font-bold ${lang === resultData.primary ? 'text-white' : 'text-slate-400'}`}>{lang}</span>
                <span className="text-xs text-slate-500">{score as number} / 20</span>
              </div>
              <div className="w-full h-2 bg-slate-800 rounded-full">
                <div
                  className={`h-full rounded-full ${lang === resultData.primary ? 'bg-pink-500' : 'bg-slate-600'}`}
                  style={{ width: `${(Number(score) / 20) * 100}%` }} // Max score is 20
                />
              </div>
            </div>
          ))}
